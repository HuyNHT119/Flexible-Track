...
export class LanguagesComponent implements OnInit, OnDestroy {
    ...
    private _updateNavigation(lang: string): void {
        ...
        // Get the Example item and update its title
        const exampleItem = this._fuseNavigationService.getItem('scrumboard.example', navigation);
        if (exampleItem) {
            this._translocoService.selectTranslate('Example').pipe(take(1))
                .subscribe((translation) => {

                    // Set the title
                    exampleItem.title = translation;
        ...
        const sampleItem = this._fuseNavigationService.getItem('scrumboard.sample', navigation);
        if (sampleItem) {
